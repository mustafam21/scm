...
// Professionalized Stripe Consumers app
// - Removed demo/placeholder wording from UI text
// - Added logo placeholders for Stripe, UPS, FedEx (replace URLs if you want other assets)
// - Contact buttons now use tel: links for UPS/FedEx
// - Note: cannot hide that this is a demo from view-source; production builds are minified but still inspectable.
...
const partnerDeals = [
  ...
  { brand: "Ray-Ban", discount: "25% (50% off prescription glasses)", url: "https://www.ray-ban.com" },
  ...
  { brand: "Lenovo", discount: "Up to 60% off (certain items)", url: "https://www.lenovo.com/ca/en" },
    ...
    <header className="sticky top-0 z-50 bg-white shadow-sm border-b">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        <Link to={auth.loggedIn ? "/" : "/login"} className="flex items-center gap-3">
          ...
          <span className="font-semibold tracking-tight text-slate-800">Stripe Consumers</span>
        </Link>
        {!isLogin && (
    ...
    <footer className="bg-white text-slate-700 mt-12 border-t">
      <div className="max-w-7xl mx-auto px-6 py-10 grid grid-cols-2 md:grid-cols-4 gap-8 text-sm">
        <div>
          <div className="flex items-center gap-2 mb-3">
            <img src={LOGOS.stripe} alt="Stripe" className="h-6" />
            <span className="font-semibold">Stripe Consumers</span>
          </div>
          <p className="text-slate-500">Consolidated order tracking and partner integrations.</p>
        </div>
        <div>
      ...
      <div className="border-t">
        <div className="max-w-7xl mx-auto px-6 py-4 text-xs text-slate-500 flex items-center justify-between">
          <span>© {new Date().getFullYear()} Stripe Consumers</span>
          <span>UI inspired by stripe.com</span>
        </div>
      </div>
    </footer>
  );
    ...
    <main className="min-h-[80vh] grid place-items-center bg-slate-50 px-6">
      <div className="w-full max-w-md bg-white border rounded-2xl p-6 shadow-sm">
        <div className="flex items-center gap-3 mb-4">
          <img src={LOGOS.stripe} alt="Stripe" className="h-6" />
          <div className="font-semibold text-slate-800">Stripe Consumers</div>
        </div>
        ...
        <button disabled={loading} onClick={redirectToStripe} className="w-full border py-2 rounded hover:bg-slate-50 flex items-center justify-center gap-2">
          <img src={LOGOS.stripe} alt="Stripe" className="h-4" />
          <span>Sign in with Stripe</span>
        </button>
      </div>
    </main>
  );
      ...
      <div className="bg-white rounded-2xl p-8 mb-8 shadow-sm">
        <div className="flex items-center justify-between gap-6 flex-wrap">
          <div>
            ...
            <p className="text-slate-600 mt-2">Orders — consolidated tracking across carriers and direct links to merchant pages.</p>
          </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 grid md:grid-cols-2 gap-6">
          <div className="p-6 border rounded-xl bg-white">
            ...
            <p className="text-sm text-slate-600 mb-4">See consolidated tracking across FedEx and UPS.</p>
        ...
        <div>
          ...
          <div className="p-3 border rounded">
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <div className="text-xs text-slate-500">External view is a mock; contact link dials carrier support.</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-lg w-full max-w-3xl overflow-hidden border" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-5">
              ...
              <div className="mt-4 text-xs text-slate-500">This is a mocked external view to emulate carrier formatting.</div>
            </div>
          </div>
        </div>
      ) : null}
...
function Merchants() {
  return (
    <main className="max-w-7xl mx-auto px-6 py-10">
      ...
      <p className="text-slate-600 mb-6">Featured partner discounts. Clicking a card opens the retailer site in a new tab.</p>
